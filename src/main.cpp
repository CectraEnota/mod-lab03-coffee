...
void displayState(const Automata& machine) {
    ...
    std::cout << "Минимальная сумма для внесения: " << machine.getMinCoin() << " руб." << std::endl;
}
...
int main()
{
    ...
    if (machine.coin(20)) {
        ...
        std::cout << "\nПытаемся добавить 10 рублей (меньше минимальных 20): ";
        if (!machine.coin(10)) {
            std::cout << "Ошибка: минимальная сумма - " << machine.getMinCoin() << " руб.\n";
        }
        ...
        if (machine.choice(1)) {
            std::cout << "Напиток готов! Сдача: " << machine.getBalance() << " руб.\n";
        }
        displayState(machine);
    }
    machine.off();
